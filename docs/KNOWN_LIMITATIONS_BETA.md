...
- Part and voice mapping is intentionally explicit. When both hands share one MusicXML part, separate voice IDs are required for an unambiguous hand assignment.
- Fingering contained in supported MusicXML is displayed where the renderer supports it. The limited editor can override fingers for generated expected-event IDs; it does not rewrite or export the original notation.
